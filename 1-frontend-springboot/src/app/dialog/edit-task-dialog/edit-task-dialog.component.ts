...
export class EditTaskDialogComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.newTitle = this.task.title as string;

    // чтобы в html странице корректно работали выпадающие списки - лучше работать не с объектами, а с их id
    if (this.task.priority) {
      this.newPriorityId = this.task.priority.id as number;
    }

    if (this.task.category) {
      this.newCategoryId = this.task.category.id as number;
      this.oldCategoryId = this.task.category.id as number; // старое значение категории всегда будет храниться тут
    }
  ...
  confirm(): void {
    ...
    this.task!.title = this.newTitle as string;
  ...
  private findPriorityById(tmpPriorityId: number): Priority {
    return this.priorities?.find(t => t.id === tmpPriorityId) as Priority;
  }

  // поиск категории по id
  private findCategoryById(tmpCategoryId: number): Category {
    return this.categories?.find(t => t.id === tmpCategoryId) as Category;
  }
